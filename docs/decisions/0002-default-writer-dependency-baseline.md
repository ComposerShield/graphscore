...
`SDL_OPENGL` (Linux), `SDL_RENDER_D3D11` (Windows), and `SDL_RENDER_METAL`
(macOS), reading back the *option* GraphScore itself declared is not
sufficient evidence that a working renderer resulted, because all three
resolve through a compiled probe (`HAVE_OPENGL`, `HAVE_D3D11_H`,
`HAVE_FRAMEWORK_METAL`) that can fail independently of the option value —
...
back exactly the variables it declared — which for these three is the wrong
signal on its own.
