...
|---|---|---|
| ThorVG vs. owned render-list | **Resolved 2026-08-03**: ThorVG accepted as the production rasterization backend (ADR 0002 §A2, 2026-08-03 amendment). The Meson/Ninja toolchain pins §3a's blocker required are recorded in ADR 0002 §A1. `graphscore_rendering` implements vector rendering via ThorVG through a Meson-driven `ExternalProject_Add`, consumed as an `IMPORTED` static library — not the GraphScore-authored-CMake alternative ADR 0002 §A2.1 evaluated and rejected. The owned render-list/tessellation fallback (ADR 0002 §3b) remains GraphScore-owned code and the documented regression path if a future ThorVG pin regresses the build gate, but is no longer the production path; ADR 0004 §1's "confirmed... active Phase C path" language for the owned rasterizer is superseded for production accordingly (see that ADR's 2026-08-03 note). This resolution does not alter §2's permitted-edge matrix: `cmake/architecture_contract.cmake` already lists `thorvg` under `GRAPHSCORE_EXTERNAL_EDGES_graphscore_rendering`; no edge amendment is required. | M05 Phase 1 (this amendment); CMake bring-up in a later M05 round |
