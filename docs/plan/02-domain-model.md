...
- [ ] Stable notehead, chord, full-measure, arbitrary musical-time range, node, connector, route-segment, staff-focus, and insertion-caret selection, with explicit staff/track/voice scope. **(Phase 8f. Three scope decisions, locked with Adam before dispatch — see M02_HANDOFF.md "Phase 8f scope (locked)".) (1) Notehead selection requires adding `NotationEntityId` to `ChordNote`, which today has no id unlike every other selectable notation entity; that is a domain data-model change with an M03 persistence round-trip obligation. (2) `route-segment` is DEFERRED to M06: `RouteGeometry` stores only interior waypoints, endpoints are a rendering concern, an automatic route stores no geometry at all, and any index dies on every route edit — so segment geometry is only well-defined once the canvas computes drawn paths, which is where M06 owns connector-segment drag. This box therefore stays unchecked after 8f. (3) `staff-focus` is DROPPED as a selection kind: every plan-doc use of "focus" is context for another operation rather than an end state, and the focused staff is already the `(TrackId, StaveId)` scope carried by every other kind, with the "nothing selected here" case being the insertion caret. A `Selection` is a set of same-kind items, per the node multi-select requirement in `06-graph-canvas.md:63`.**
