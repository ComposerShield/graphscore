...
- [ ] Stable notehead, chord, full-measure, arbitrary musical-time range, node, connector, and insertion-caret selection, with explicit staff/track/voice scope. **(Phase 8f. Locked decisions — see M02_HANDOFF.md "Phase 8f-ii: Selection representation".) A `Selection` is a `std::variant` of non-empty homogeneous SETS OF SCOPED ITEMS — each variant arm carries per-item scope (not one shared scope plus payload), supporting aligned multi-track/stave/voice and multi-node selections without Cartesian ambiguity. `route-segment` is DEFERRED to M06 (geometry only becomes well-defined once the canvas computes drawn paths). `staff-focus` is DROPPED as a selection kind (context for another operation, not an end state). 8f-i ChordNote + GraceNote identity groundwork is complete (stable `NotationEntityId`s, voice-scoped cross-kind/nil collision guards, M03 round-trip obligation). 8f-ii selection representation remains outstanding.**
