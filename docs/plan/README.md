...
- Keyboard-only operation remains a first-release requirement. Platform
  accessibility bridges, screen-reader support, assistive-technology
  validation, and accessibility-specific release gates are deferred
  indefinitely and are not part of `0.1.0` or any currently scheduled
  milestone. The toolkit-neutral semantic accessibility code already present
  in the repository is retained, but it is not a supported user-facing
  contract and does not imply VoiceOver, Narrator, or Orca integration.
...
- Zoom is centered on the pointer or keyboard focus as appropriate.
...
- Native VST3 editors are supported. A GraphScore-owned generic parameter view is provided where practical.
...
Progress is tracked in the source-controlled [execution checklist](CHECKLIST.md), where every milestone and major implementation/test step has its own task box.
...
| [05](05-notation-editor.md) | Focused notation engraving, editing, and keyboard workflows |
...
| [10](10-hardening.md) | Realtime, performance, compatibility, and resilience release gates |
...
- Update schemas, C API documentation, and user-facing behavior documentation with the implementation.
