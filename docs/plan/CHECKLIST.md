...
- [x] &nbsp;&nbsp;Rest/marking selection-model increment completed<br>&nbsp;&nbsp;&nbsp;&nbsp;(`RestItem`/`RestSet` and `MarkingItem`/`MarkingSet` added to `Selection`, which now has nine arms, plus<br>&nbsp;&nbsp;&nbsp;&nbsp;`MarkingKind` and `validate_selection` coverage for each. Markings are addressed by *composite key* —<br>&nbsp;&nbsp;&nbsp;&nbsp;anchor entity id plus kind, plus an `Articulation` discriminator — not by minting an id per marking:<br>&nbsp;&nbsp;&nbsp;&nbsp;articulations, ties and tuplets are values on `Note`/`ChordNote`/`Duration`, so giving them ids would<br>&nbsp;&nbsp;&nbsp;&nbsp;change three widely-used aggregates and every persistence/clipboard/remapping path for no user-visible<br>&nbsp;&nbsp;&nbsp;&nbsp;gain; `Note`, `ChordNote` and `Duration` are untouched and no production code mints a `Uuid`.<br>&nbsp;&nbsp;&nbsp;&nbsp;`BeamOverride` is deliberately excluded — it has an id but the engraver emits no hit geometry for it.<br>&nbsp;&nbsp;&nbsp;&nbsp;`kDuplicateArticulation` added to `NotationDiagnosticCode` and `DiagnosticCode`, which makes the composite<br>&nbsp;&nbsp;&nbsp;&nbsp;key unambiguous but also means six M02 command paths now reject input they previously accepted.<br>&nbsp;&nbsp;&nbsp;&nbsp;M03 obligation: `Rest::id`, every marking record's id, and — because of composite addressing —<br>&nbsp;&nbsp;&nbsp;&nbsp;articulation membership, `tied_to_next`, and tuplet run boundaries are all load-bearing across a<br>&nbsp;&nbsp;&nbsp;&nbsp;migration. Reviewer-approved: debug 1980/1980, full lint, seven audits, clang-tidy 18, ASan/UBSan.)
- [x] &nbsp;&nbsp;Pointer selection-resolver increment completed<br>&nbsp;&nbsp;&nbsp;&nbsp;(`resolve_selection_at` maps a point to the one `Selection` it names: notehead → `NoteheadSet`,<br>&nbsp;&nbsp;&nbsp;&nbsp;whole event → `ChordSet`/`RestSet`, staff-level or unmatched → `InsertionCaretSet` in the armed voice.<br>&nbsp;&nbsp;&nbsp;&nbsp;Stems emitted no hit region at all before this, so nothing named a whole chord; they now emit one with<br>&nbsp;&nbsp;&nbsp;&nbsp;`HitRole::kEvent` rather than a new role, since `kEvent` already means "the whole event" and `kNotehead`<br>&nbsp;&nbsp;&nbsp;&nbsp;"one pitch" — notehead-versus-stem then falls out of `hit_test`'s existing priority order with no enum<br>&nbsp;&nbsp;&nbsp;&nbsp;change. Typed ids come from the layout tree (`NotationLayout::node_id`, `StaffSystemLayout`'s<br>&nbsp;&nbsp;&nbsp;&nbsp;`track_id`/`stave_id`, `VoiceLayout::voice`), never from parsing strings — no UUID parser exists — and a<br>&nbsp;&nbsp;&nbsp;&nbsp;hit's owning staff is found by scanning for the staff whose voice content holds the entity id, because<br>&nbsp;&nbsp;&nbsp;&nbsp;deriving it from the click point discards ledger-line noteheads outside the six-staff-space window and<br>&nbsp;&nbsp;&nbsp;&nbsp;mis-attributes across a grand staff. The caret is gated against the domain's own legality rule before<br>&nbsp;&nbsp;&nbsp;&nbsp;construction: an empty armed voice resolves its onset from a hypothetical measure-aligned rest fill,<br>&nbsp;&nbsp;&nbsp;&nbsp;legal for a preview but not an event boundary, so the ordinary "start a new voice on an existing score"<br>&nbsp;&nbsp;&nbsp;&nbsp;click would otherwise produce a `Selection` `validate_selection` rejects. Every returned `Selection`<br>&nbsp;&nbsp;&nbsp;&nbsp;satisfies `validate_selection(...).empty()`, asserted per case. Hit-id suffixes are shared `constexpr`<br>&nbsp;&nbsp;&nbsp;&nbsp;constants used at both emitter and resolver, matched at a path boundary, so an emitter cannot drift<br>&nbsp;&nbsp;&nbsp;&nbsp;silently. `preview_note_entry`'s staff/measure/onset resolution factored into shared helpers, verified<br>&nbsp;&nbsp;&nbsp;&nbsp;behaviour-preserving branch by branch against `HEAD`. Reviewer-approved exact tree: debug 2003/2003,<br>&nbsp;&nbsp;&nbsp;&nbsp;full lint, all seven audits, clang-tidy 18 writer-OFF 113/113 zero findings, ASan/UBSan 2003/2003 with<br>&nbsp;&nbsp;&nbsp;&nbsp;11 expected writer-resource skips. Remaining in this bullet: marking resolution for all seven<br>&nbsp;&nbsp;&nbsp;&nbsp;`MarkingKind`s, and range selections. Two known items carried forward — a cross-system tuplet bracket<br>&nbsp;&nbsp;&nbsp;&nbsp;can be anchored to a mid-run event that `validate_selection` rejects, so the marking resolver must<br>&nbsp;&nbsp;&nbsp;&nbsp;normalize to the true run start; and a stemless whole note/whole-note chord has no `kEvent` geometry at<br>&nbsp;&nbsp;&nbsp;&nbsp;all, so it resolves only through its noteheads — documented on the contract, awaiting a product<br>&nbsp;&nbsp;&nbsp;&nbsp;decision rather than closed by inventing an affordance.)
- [x] &nbsp;&nbsp;Marking selection-resolver increment completed<br>&nbsp;&nbsp;&nbsp;&nbsp;(`resolve_selection_at` now resolves a `HitRole::kMarking` hit to a one-item `MarkingSet` for all seven<br>&nbsp;&nbsp;&nbsp;&nbsp;`MarkingKind`s, where it previously returned `nullopt`. The seven are *not* discriminated uniformly:<br>&nbsp;&nbsp;&nbsp;&nbsp;dynamic, hairpin, slur and pedal span are real records whose own id *is* the hit's `semantic_id`, so the<br>&nbsp;&nbsp;&nbsp;&nbsp;kind is determined by looking that id up in `dynamics()`/`hairpins()`/`slurs()`/`pedal_spans()` — a<br>&nbsp;&nbsp;&nbsp;&nbsp;successful lookup in disjoint id spaces *is* the discriminator, which no emitter rename can silently<br>&nbsp;&nbsp;&nbsp;&nbsp;break. String matching is confined to articulation, tie and tuplet, which have no ids of their own and<br>&nbsp;&nbsp;&nbsp;&nbsp;share both `kMarking` and an event-id `semantic_id`, so only the hit id's shape can separate them;<br>&nbsp;&nbsp;&nbsp;&nbsp;`kHitRoleArticulation`/`kHitRoleTie`/`kHitRoleTupletDigit` are shared constants used at both emitter and<br>&nbsp;&nbsp;&nbsp;&nbsp;resolver, matched at path boundaries. `hit_id_ends_with` could not be reused — the variable<br>&nbsp;&nbsp;&nbsp;&nbsp;`system-N`/index infixes break its anchoring. The `MarkingItem` shape rules are enforced by<br>&nbsp;&nbsp;&nbsp;&nbsp;`MarkingSet::create` *silently* (it returns `nullopt`, not a diagnostic): `voice` is disengaged for<br>&nbsp;&nbsp;&nbsp;&nbsp;`kPedalSpan` because pedal is stave-scoped and engaged for every other kind, and `articulation` is<br>&nbsp;&nbsp;&nbsp;&nbsp;engaged only for `kArticulation`, so getting either backwards makes the selection vanish rather than<br>&nbsp;&nbsp;&nbsp;&nbsp;error. Cross-system tuplet hazard closed: the engraver prepends only one measure of lookback when<br>&nbsp;&nbsp;&nbsp;&nbsp;scanning a system fragment, so a continued run's bracket can be anchored to a *mid-run* event that<br>&nbsp;&nbsp;&nbsp;&nbsp;`validate_selection` rejects; `normalize_tuplet_anchor` walks the full voice backward to the run's true<br>&nbsp;&nbsp;&nbsp;&nbsp;first event. Fixed in the resolver, not the engraver, which cannot know — it has one measure of lookback<br>&nbsp;&nbsp;&nbsp;&nbsp;by construction. Articulation ids carry the true `event_articulations` vector index, not a drawn-glyph<br>&nbsp;&nbsp;&nbsp;&nbsp;ordinal: the emitter suppresses duplicate duration articulations, so emitted indices have gaps and<br>&nbsp;&nbsp;&nbsp;&nbsp;counting glyphs would select the wrong articulation. Every returned `Selection` satisfies<br>&nbsp;&nbsp;&nbsp;&nbsp;`validate_selection(...).empty()`, asserted unconditionally per case. Reviewer reproduced rather than<br>&nbsp;&nbsp;&nbsp;&nbsp;accepted: normalization proven load-bearing by disabling the backward walk (anchor `notes[11]` →<br>&nbsp;&nbsp;&nbsp;&nbsp;`"event is not the first event of its tuplet run"`), a 36-event run over three systems normalizing from<br>&nbsp;&nbsp;&nbsp;&nbsp;two different mid-run anchors, and a grand-staff sweep of 84 marking regions → 609 resolutions with zero<br>&nbsp;&nbsp;&nbsp;&nbsp;`nullopt` and zero validation failures. Reviewer-approved exact tree: debug 2015/2015, notation 206/206,<br>&nbsp;&nbsp;&nbsp;&nbsp;full lint, all seven audits, clang-tidy 18 writer-OFF, ASan/UBSan 2015/2015. Remaining in the parent<br>&nbsp;&nbsp;&nbsp;&nbsp;bullet: range selections, gated on the selection tool and measure targets below, and the stemless<br>&nbsp;&nbsp;&nbsp;&nbsp;whole-note/whole-note-chord decision, which is a product call rather than a task. Three items carried<br>&nbsp;&nbsp;&nbsp;&nbsp;forward, none introduced here: marking lookup is O(systems × markings) with a `to_string()` per<br>&nbsp;&nbsp;&nbsp;&nbsp;comparison, measured 1.4 ms on a 200-system score — off the realtime path and matching the existing<br>&nbsp;&nbsp;&nbsp;&nbsp;`resolve_hit_entity` shape, so deliberately not optimized on speculation; a tie hit region at priority 5<br>&nbsp;&nbsp;&nbsp;&nbsp;shadows the articulation glyphs on a chord carrying both, so those articulations are not clickable —<br>&nbsp;&nbsp;&nbsp;&nbsp;confirmed to be `hit_test` geometry, not resolver misclassification, and it belongs to the hit-priority<br>&nbsp;&nbsp;&nbsp;&nbsp;work below; and the engraver suppresses tuplet digits per-system against the *local* first record while<br>&nbsp;&nbsp;&nbsp;&nbsp;the domain keys `kIncompleteTupletGroup` to the *true global* run start, which can disagree the same way<br>&nbsp;&nbsp;&nbsp;&nbsp;the anchor did.)
- [x] &nbsp;&nbsp;Pointer-drag range-resolver increment completed<br>&nbsp;&nbsp;&nbsp;&nbsp;(`resolve_range_selection` maps a two-point drag to an `ArbitraryRangeSet`: one `ArbitraryRangeItem` per<br>&nbsp;&nbsp;&nbsp;&nbsp;`(track, stave, voice)`, every item carrying an *identical* `MusicalSpan`, which `extract_arbitrary_range`<br>&nbsp;&nbsp;&nbsp;&nbsp;requires and rejects with `kInvalidArgument` otherwise. This is the repository's first producer of a<br>&nbsp;&nbsp;&nbsp;&nbsp;multi-item `Selection` of any arm, and the first positive multi-item `extract_arbitrary_range` test —<br>&nbsp;&nbsp;&nbsp;&nbsp;only the mixed-span rejection was covered before. Takes no `NotePaletteState` by design: this is a<br>&nbsp;&nbsp;&nbsp;&nbsp;dedicated selection tool, so the palette's armed voice must not filter it. Staves are those between the<br>&nbsp;&nbsp;&nbsp;&nbsp;two endpoints in score order, inclusive — not a rectangle intersection, which is ambiguous across a<br>&nbsp;&nbsp;&nbsp;&nbsp;system break where each system carries different staves. Voices are content-derived (a voice is included<br>&nbsp;&nbsp;&nbsp;&nbsp;iff an event's `[onset, onset + duration)` overlaps the span), because `VoiceLayout` carries no geometry<br>&nbsp;&nbsp;&nbsp;&nbsp;at all and emitting four items on a single-voice score would be wrong. The span is ordered by *musical<br>&nbsp;&nbsp;&nbsp;&nbsp;time, not x*, so a backwards or upward drag and a drag spanning a system break both produce one<br>&nbsp;&nbsp;&nbsp;&nbsp;contiguous range. Endpoints quantize to a 1/192 whole-note grid (`lcm(64, 3)`), exact for plain binary<br>&nbsp;&nbsp;&nbsp;&nbsp;values through the sixty-fourth and plain triplets, inexact for a dotted sixty-fourth (3/128, 7/256) and<br>&nbsp;&nbsp;&nbsp;&nbsp;higher-prime tuplets; that is acceptable because the tool deliberately does **not** snap to event onsets —<br>&nbsp;&nbsp;&nbsp;&nbsp;snapping would make the domain's head/tail-straddle clipping rules R1–R12 unreachable from the pointer,<br>&nbsp;&nbsp;&nbsp;&nbsp;and quantization moves only the drag endpoint, never rhythmic content. Reviewer reproduced non-vacuity<br>&nbsp;&nbsp;&nbsp;&nbsp;with three independent mutants against the real tests: skipping interior staves, over-including staves<br>&nbsp;&nbsp;&nbsp;&nbsp;outside the range, and dropping the `std::minmax` ordering each fail exactly one case and no other. Review<br>&nbsp;&nbsp;&nbsp;&nbsp;found no behavioural defect; it rejected the first round purely on evidence quality — a naive<br>&nbsp;&nbsp;&nbsp;&nbsp;endpoints-only implementation passed the entire original suite, since every fixture had at most two<br>&nbsp;&nbsp;&nbsp;&nbsp;staves. A `bugprone-unchecked-optional-access` suppression was removed rather than justified:<br>&nbsp;&nbsp;&nbsp;&nbsp;`Rational(n) / Rational(192)` is value-identical to `*Rational::create(n, 192)` and needs no `NOLINT`. The<br>&nbsp;&nbsp;&nbsp;&nbsp;one remaining suppression is unavoidable — `Voice`'s converting constructor is private. Reviewer-approved<br>&nbsp;&nbsp;&nbsp;&nbsp;exact tree: debug 2027/2027, notation 218/218, full lint, all seven audits, clang-tidy 18 writer-OFF over<br>&nbsp;&nbsp;&nbsp;&nbsp;both the production and test translation units, ASan/UBSan 2027/2027. Remaining in this bullet: app-layer<br>&nbsp;&nbsp;&nbsp;&nbsp;drag state and an active-tool discriminator, plus wiring pointer press/move/release — none of which exist<br>&nbsp;&nbsp;&nbsp;&nbsp;yet, as `WriterShell` currently only opens an SDL window — and a live selection extent during the drag.<br>&nbsp;&nbsp;&nbsp;&nbsp;The resolver is a pure per-frame query, so a caller can drive a live preview by calling it on every<br>&nbsp;&nbsp;&nbsp;&nbsp;pointer move; nothing renders an `ArbitraryRangeSet`'s extent today.)
