...
static_assert(sizeof(gs_result_t) == 12, "gs_result_t size mismatch");

TEST(RuntimeTest, VersionQueryReturnsExpected) {
  const gs_version_t v = gs_query_version();
...
TEST(RuntimeTest, CreateNullOutPlayerFails) {
  const gs_result_t r = gs_player_create(nullptr, nullptr);
  EXPECT_EQ(r.code, GS_RESULT_INVALID_ARGUMENT);
}

TEST(RuntimeTest, PollDiagnosticsReturnsZero) {
  gs_diagnostics_t  d{};
  const gs_result_t r = gs_player_poll_diagnostics(nullptr, &d);
  EXPECT_EQ(r.code, GS_RESULT_SUCCESS);
...
TEST(RuntimeTest, QueryCapacityReturnsSuccess) {
  uint32_t          caps = 0;
  const gs_result_t r    = gs_player_query_capacity(nullptr, 256, 16, &caps);
  EXPECT_EQ(r.code, GS_RESULT_SUCCESS);
}
...
TEST(RuntimeTest, ProcessReturnsSuccess) {
  ...
  const gs_result_t r = gs_player_process(nullptr, &params);
  EXPECT_EQ(r.code, GS_RESULT_SUCCESS);
}
