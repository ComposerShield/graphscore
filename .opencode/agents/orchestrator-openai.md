...
- The initial worker-sol must self-audit every phase requirement and locked deliverable and
...
- Before Tier 2, if GraphScore-owned C/C++ production code changed, require focused canonical
  ...
  commands in `AGENTS.md`. The worker-sol reports exact targets and results. Docs/config-only
  work reports N/A. This early check does not replace Tier 3 clang-tidy.
- Screen the worker-sol report before review. If any requirement row, implementation reference,
...
1. Dispatch a reviewer in `INITIAL_FULL_REVIEW` mode with the plan, complete phase scope,
   changed paths, and the worker-sol's **full traceability matrix**. It audits the complete phase
...
2. If the initial reviewer reports findings, do not run Tier 3. Send a fix worker-sol only the
   findings, affected traceability rows, and equivalent defect family—not the full matrix.
   A small immediate fix may resume the same worker-sol; substantial work or a long session gets
   a fresh worker-sol and self-contained prompt. Require only targeted Tier 1 regressions and
...
3. Dispatch a **fresh reviewer for unbiased targeted validation** in
   `TARGETED_REREVIEW` mode. Provide prior findings, the fix worker-sol's changed paths/delta,
