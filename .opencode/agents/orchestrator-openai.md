...
2. Work the milestone's **phases strictly in order**. For each phase:
   ...
   - Only after approval: have the worker-terra check off the phase's `- [ ]` boxes in the plan
...
- Use the `explore` subagent for research/reconnaissance (codebase questions, prior art in
  `../soul-redux`, format investigations) before writing worker-terra prompts that depend on it.
