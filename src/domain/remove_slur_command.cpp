...
Result RemoveSlurCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.remove_slur(marking_id_);
    if (!result.ok())
      return result;
...
Result RemoveSlurCommand::undo(Project& project) noexcept {
...
Result RemoveSlurCommand::redo(Project& project) noexcept {
