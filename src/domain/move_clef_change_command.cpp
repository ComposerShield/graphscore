...
Result MoveClefChangeCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    const Result candidate_result = candidate.move_change(from_, to_);
    if (!candidate_result.ok())
      return candidate_result;
    const Result result = timeline->move_clef_change(stave_id_, from_, to_);
    if (!result.ok())
      return result;
...
Result MoveClefChangeCommand::undo(Project& project) noexcept {
...
Result MoveClefChangeCommand::redo(Project& project) noexcept {
