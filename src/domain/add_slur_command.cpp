...
Result AddSlurCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.add_slur(slur_);
    if (!result.ok())
      return result;
...
Result AddSlurCommand::undo(Project& project) noexcept {
...
Result AddSlurCommand::redo(Project& project) noexcept {
