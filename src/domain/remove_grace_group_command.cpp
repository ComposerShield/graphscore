...
Result RemoveGraceGroupCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.remove_grace_group(marking_id_);
    if (!result.ok())
      return result;
...
Result RemoveGraceGroupCommand::undo(Project& project) noexcept {
...
Result RemoveGraceGroupCommand::redo(Project& project) noexcept {
