...
Result RemoveBeamOverrideCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.remove_beam_override(marking_id_);
    if (!result.ok())
      return result;
...
Result RemoveBeamOverrideCommand::undo(Project& project) noexcept {
...
Result RemoveBeamOverrideCommand::redo(Project& project) noexcept {
