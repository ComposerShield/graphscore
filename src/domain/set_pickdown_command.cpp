...
Result SetPickdownCommand::execute(Project& project) noexcept {
  ...
  try {
    std::optional<internal::PickdownTempoSnapshots> tempo =
        internal::prepare_pickdown_tempo_snapshots(
            *timeline, timeline->boundary_position() + duration_);
    if (!tempo.has_value())
      return Result(ResultCode::kInvalidArgument);
    const std::optional<Rational> pre    = timeline->pickdown_duration();
    const Result                  result = timeline->set_pickdown(duration_);
    if (!result.ok())
      return result;
    pre_snapshot_        = pre;
    post_snapshot_       = duration_;
...
Result SetPickdownCommand::undo(Project& project) noexcept {
...
Result SetPickdownCommand::redo(Project& project) noexcept {
