...
Result RemovePedalSpanCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.remove_pedal_span(stave_id_, marking_id_);
    if (!result.ok())
      return result;
...
Result RemovePedalSpanCommand::undo(Project& project) noexcept {
...
Result RemovePedalSpanCommand::redo(Project& project) noexcept {
