...
Result AddGraceGroupCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.add_grace_group(grace_group_);
    if (!result.ok())
      return result;
...
Result AddGraceGroupCommand::undo(Project& project) noexcept {
...
Result AddGraceGroupCommand::redo(Project& project) noexcept {
