...
Result AddBeamOverrideCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.add_beam_override(beam_override_);
    if (!result.ok())
      return result;
...
Result AddBeamOverrideCommand::undo(Project& project) noexcept {
...
Result AddBeamOverrideCommand::redo(Project& project) noexcept {
