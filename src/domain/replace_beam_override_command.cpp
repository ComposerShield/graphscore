...
Result ReplaceBeamOverrideCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.replace_beam_override(marking_id_, replacement_);
    if (!result.ok())
      return result;
...
Result ReplaceBeamOverrideCommand::undo(Project& project) noexcept {
...
Result ReplaceBeamOverrideCommand::redo(Project& project) noexcept {
