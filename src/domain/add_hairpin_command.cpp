...
Result AddHairpinCommand::execute(Project& project) noexcept {
  ...
  try {
    ...
    Result result = candidate.add_hairpin(hairpin_);
    if (!result.ok())
      return result;
...
Result AddHairpinCommand::undo(Project& project) noexcept {
...
Result AddHairpinCommand::redo(Project& project) noexcept {
