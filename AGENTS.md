...
| `apps/` | Application entry points (`graphscore_writer_app`, `graphscore_plugin_scanner`). The writer's assembly is split across `apps/writer_app/` (private headers and translation units) and `apps/writer_app/selftests/` (the 17 `--test-*` self-tests); neither is a target of its own — they all compile into `graphscore_writer_app`. |
