...
| `tests/<target>/` | GTest sources for `graphscore_<target>_test`, plus `tests/c_abi/` (pure-C ABI consumer), `tests/cmake/` (out-of-tree consumer projects driven against a real install tree), and `tests/repository/` (checkout and build-system properties). `tests/domain/command/` holds the `CommandTest` suite's translation units and their shared private headers; it is not a target of its own — they all compile into `graphscore_domain_test`. |
