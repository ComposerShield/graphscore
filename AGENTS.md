...
| `tests/<target>/` | GTest sources for `graphscore_<target>_test`, plus `tests/c_abi/` (pure-C ABI consumer), `tests/cmake/` (out-of-tree consumer projects driven against a real install tree), and `tests/repository/` (checkout and build-system properties). `tests/domain/command/` holds the `CommandTest` suite's translation units and their shared private headers; `tests/notation/selection/` holds the selection-resolver suites' (`SelectionResolverTest`, `MeasureSelectionTest`, `MeasureExtensionTest`, `RangeSelectionTest`, `RangeSpecTest`, `ScoreOrderedStavesTest`, `RangeExtensionTest`, `RangeStaffScopeExtensionTest`, `SelectionDragLifecycleTest`, `HighlightRectsTest`, `StaffStepTest`) translation units and their shared private header/source pair; `EventStyleEditTest` and `MarkingStyleEditTest` are the toolkit-neutral articulation/stem and dynamic/hairpin/pedal-span factory suites; none is a target of its own — they compile into `graphscore_domain_test` and `graphscore_notation_test`. |
| `apps/` | Application entry points (`graphscore_writer_app`, `graphscore_plugin_scanner`). The writer's assembly is split across `apps/writer_app/` (private headers and translation units) and `apps/writer_app/selftests/` (the 20 `--test-*` self-tests); neither is a target of its own — they all compile into `graphscore_writer_app`. |
